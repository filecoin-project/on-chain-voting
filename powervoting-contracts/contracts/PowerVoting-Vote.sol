...
contract PowerVoting is IPowerVoting, Ownable2StepUpgradeable, UUPSUpgradeable {
    ...
    // add f4 task function selector    //todo delete
    bytes4 public constant ADD_F4_TASK_SELECTOR =
        bytes4(keccak256("addF4Task(address)"));

    // add miner id function selector
    bytes4 public constant ADD_MINER_IDS_SELECTOR =    //todo move to oracle
        bytes4(keccak256("addMinerIds(uint64[],address)"));

    // get voter info
    bytes4 public constant GET_VOTER_INFO_SELECTOR =
        bytes4(keccak256("getVoterInfo(address)")); // todo delete
    ...
    ) public initializer nonZeroAddress(oracleAddress) {
        //uups must init here 
    ...
    * @notice set proposal content and title max length
    ...
    function updateOracleContract(
        address oracleAddress
    ) external onlyOwner nonZeroAddress(oracleAddress) {
        oracleContract = oracleAddress;
    }
  ...
  /**
    ...
    *  @param tokenHolderPercentage The token's weight when the proposal is counted. This value should be within 0 - 100 * DECIMAL_MULTIPLIER (representing 0 - 100.00 with precision determined by DECIMAL_MULTIPLIER).
     ...
     */
    function addMinerId(uint64[] memory minerIds) external override {
        _addF4Task();
        (bool addMinerSuccess, ) = oracleContract.call(
            abi.encodeWithSelector(ADD_MINER_IDS_SELECTOR, minerIds, msg.sender)
        );
        if (!addMinerSuccess) {
     ...
     */
    function addF4Task() external override {
        _addF4Task();
    }
    ...
    function _addF4Task() private {
        (bool getVoterInfoSuccess, bytes memory data) = oracleContract.call(
            abi.encodeWithSelector(GET_VOTER_INFO_SELECTOR, msg.sender)
        );
        ...
        if (voterInfo.actorIds.length == 0) {
            (bool addF4TaskSuccess, ) = oracleContract.call(
                abi.encodeWithSelector(ADD_F4_TASK_SELECTOR, msg.sender)
            );
            if (!addF4TaskSuccess) {
