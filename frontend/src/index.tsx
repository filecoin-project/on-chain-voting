...
import {
  powerVotingCalibrationChainRpc,
...
const filecoinCalibrationChain = {
  ...
  rpcUrls: {
    default: { http: [powerVotingCalibrationChainRpc] },
  },
  blockExplorers: {
    default: {
      name: "glif",
  ...
  createConnector: (walletDetails) => {
    return createConnector((config) => ({
      ...
      async connect(chain) {
        ...
        const res = await filSnapAdapter.connect({ network: "testnet" })
        const accounts = [
          (res?.account?.address.toString() as `0x${string}`) || "0x",
        ]
      ...
      async switchChain({ chainId }) {
        try {
          ...
          await filSnapAdapter.changeNetwork("testnet")
          config.emitter.emit("change", {
...
const config = createConfig({
  ...
  transports: {
    [filecoinCalibrationChain.id]: http(powerVotingCalibrationChainRpc),
  },
