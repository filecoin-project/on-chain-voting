...
func (ev *Event) parseEvent(ctx context.Context, vLog types.Log) error {
	...
	case ev.Client.ABI.PowerVotingAbi.Events[constant.ProposalEvt].ID.Hex():
		var event model.ProposalCreateEvent
		...
		snapshotInfo, err := snapshot.SyncSnapshot(ev.Client.ChainId, snapshotday)
		if err != nil {
			return fmt.Errorf("record snapshot error: %w", err)
		}

		data.SnapshotHeight = snapshotInfo.Height
