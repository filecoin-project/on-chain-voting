...
func UpdateMinerIds(client *RPCClient, from string, minerIds []uint64) (string, error) {
	encodedData, err := config.OracleAbi.Pack("updateMinerIds", minerIds)
	if err != nil {
		zap.L().Error("Failed to ABI encode minerId data", zap.Error(err))
		return "", err
	}
	...
	for {
		...
		if confirmation == "yes" {
			...
			return "", errors.New("minerId not confirmed, transaction not sent")
		} else {
